...
FeatureDict = dict[str, Union[npt.NDArray[np.float_], npt.NDArray[np.bool_]]]
...
def global_features(
  ...
  transit_mask, _, _, _, _ = preprocess.tr_mask_view(tic, time, transit_mask,
                                                     period)
  return {
      "global_view": view,
      "global_std": std,
...
def local_features(
    ...
    period: float,
    duration: float,
) -> tuple[FeatureDict, Optional[float], Optional[float]]:
  """
  ...
  return (
      {
          ...
          "local_mask":
              mask,
          "local_scale":
              np.array([scale]) if scale is not None else np.array([0.0]),
          "local_scale_present":
              np.array([scale is not None]).astype(float),
      },
      scale,
      depth,
  )
...
def secondary_features(
    ...
    scale: Optional[float],
    depth: Optional[float],
) -> tuple[FeatureDict, Optional[float]]:
  """
  ...
  """
  (_, _, _, secondary_scale,
   _), _ = preprocess.secondary_view(tic, time, flux, period, duration)
  ...
  return (
      {
          ...
          "secondary_phase":
              np.array([t0 / period]),
          "secondary_scale":
              np.array([secondary_scale])
              if secondary_scale is not None else np.array([0.0]),
          "secondary_scale_present":
              np.array([secondary_scale is not None]).astype(float),
      },
      secondary_scale,
  )
