...
parser.add_argument("--vetting_features", type=str, default="n")

parser.add_argument("--mode", type=str, choices=["triage", "vetting"], required=True)
...
def _standard_views(
    ex, tic, time, flux, period, epoc, duration, bkspace, aperture_fluxes
):
    if bkspace is None:
        tag = ""
    else:
    ...
    for k, (t, f) in aperture_fluxes.items():
        t, f, m = preprocess.detrend_and_filter(
            tic, t, f, period, epoc, duration, bkspace
        )
    ...
    (_, _, _, sec_scale, _), t0 = preprocess.secondary_view(
        tic, time, flux, period, duration
    )
    ...
    full_view = preprocess.sample_segments_view(
        tic, time, flux, fold_num, period, duration
    )
    ...
    view, std, mask, scale, _ = preprocess.local_view(
...
def create_tfrecords(
    ...
    """Expose main function for other developers."""
...
def main(_):
    ...
    def get_lightcurve(
        astro_id: int, aperture: Optional[str] = None
    ...
    boundaries = np.linspace(0, len(tce_table), FLAGS.num_shards + 1).astype(np.int)
    ...
    for start, end, file_shard in file_shards:
        _process_file_shard(
            tce_table[start:end], file_shard, get_lightcurve, get_lightcurve, FLAGS.mode
        )
