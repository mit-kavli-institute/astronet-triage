...
def loadim(f, w):
    ...
    try:
        ...
        print('load image:', e)
        im = Image.new('RGB', (maxw, maxh))
        success = False
    return ImageTk.PhotoImage(image=im), im, success
...
def main_app():
    ...
    while True:
        ...
        while (os.path.basename(fs[i]) in lbl[0]) and (os.path.basename(fs[i]) in lbl[1]):
            ...
            if i >= len(fs):
                i = 0
                break
        _, _, success = loadim(fs[i]+".page%d.png" % j, w)

        if success or not advanced:
            break
        i += 1

    tki, im, _ = loadim(fs[i]+".page%d.png" % j, w)
    ...
    def update(pageindex):
        set_status('loading')
        update_text()
        tki, _, _ = loadim(fs[i]+".Page%d.png" % pageindex, w)
