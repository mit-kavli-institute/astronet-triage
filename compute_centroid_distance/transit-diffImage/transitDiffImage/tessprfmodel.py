...
class tessPRF(object):
  ...
  def __call__(self, center_col, center_row, flux, scale_col, scale_row,
               rotation_angle):
    return self.evaluate(center_col, center_row, flux, scale_col, scale_row,
                         rotation_angle)
  ...
  def gradient(self,
    ...
    interp_dy = self.interpolate(
        ...
        rot_col.flatten() * scale_col,
        grid=False,
        dy=1).reshape(self.shape)

    interp_dx = self.interpolate(
        rot_row.flatten() * scale_row,
        rot_col.flatten() * scale_col,
        grid=False,
        dx=1).reshape(self.shape)
    ...
    deriv_rotation_angle = flux * (
        interp_dy * scale_col *
        (delta_row * cosa - delta_col * sina) - interp_dx * scale_row *
        (delta_row * sina + delta_col * cosa))

    return [
        deriv_center_col, deriv_center_row, deriv_flux, deriv_scale_col,
        deriv_scale_row, deriv_rotation_angle
    ]
  ...
  def _prepare_prf(self):
    ...
    if self.sector < 4:
      ...
      if ((self.camera == 1) | ((self.camera == 2) & (self.ccd < 4))):
        prfName = "tess2018243163600-prf-" + str(self.camera) + "-" + str(
            self.ccd)
      else:
        prfName = "tess2018243163601-prf-" + str(self.camera) + "-" + str(
            self.ccd)
    else:
      prfSector = 4
      if (self.camera == 1) & (self.ccd == 1):
        prfName = "tess2019107181900-prf-" + str(self.camera) + "-" + str(
            self.ccd)
      elif ((self.camera == 1) & (self.ccd > 1)) \
          | (self.camera == 2) \
          | ((self.camera == 3) & (self.ccd == 1)):
        prfName = "tess2019107181901-prf-" + str(self.camera) + "-" + str(
            self.ccd)
      else:
        prfName = "tess2019107181902-prf-" + str(self.camera) + "-" + str(
            self.ccd)

    if self.prfFileLocation is None:
      prfs_url_path = "http://archive.stsci.edu/missions/tess/models/prf_fitsfiles/"
      prfLocation = prfs_url_path + "start_s000" + str(
          prfSector) + "/cam" + str(self.camera) + "_ccd" + str(self.ccd) + "/"
    else:
    ...
    for i in range(len(rowPrfAtArray)):
      for j in range(len(colPrfAtArray)):
        prffile = prfFileStub + "-row" + "{:04d}".format(
            rowPrfAtArray[i]) + "-col" + "{:04d}".format(
                colPrfAtArray[j]) + ".fits"
        #                print(prffile)
        prfn[i], crval1p[i], crval2p[i], cdelt1p[i], cdelt2p[
            i] = self._read_prf_calibration_file(prffile)
